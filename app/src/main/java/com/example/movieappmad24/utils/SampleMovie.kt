...
fun getSampleMoviesWithImages(): List<MovieWithImages> {
    var sampleImageDbId: Long = 0
    val sampleMovies = getSampleMovies()

    val sampleMoviesWithImages = mutableListOf<MovieWithImages>()
    ...
    for (sampleMovie in sampleMovies) {
        val movie = Movie(
            ...
            isFavorite = sampleMovie.isFavorite
        )

        val sampleImagesForMovie = mutableListOf<MovieImage>()

        for (image in sampleMovie.images) {
            val sampleImage = MovieImage(sampleImageDbId++, sampleMovie.id, image)
            sampleImagesForMovie.add(sampleImage)
        }

        val movieWithImages = MovieWithImages(movie, sampleImagesForMovie)

        sampleMoviesWithImages.add(movieWithImages)
    }

    return sampleMoviesWithImages
}

fun getSampleMovies(): List<SampleMovie> {
    return listOf(
        SampleMovie(
            id = "tt0499549",
            title = "Avatar",
